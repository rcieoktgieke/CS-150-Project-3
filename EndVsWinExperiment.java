...
public class EndVsWinExperiment {
    ...
    public static void main(String[] args) {
        for (int i = 0; i < 90; i ++) {
            ...
            for (int x = 110; x < 120; x ++) {
                ...
                new Thread() {
                    public void run() {
                        File file = new File("Experiment3/output" + j + " - " + y + ".csv");
                        try {
                            EndVsWinExperiment e = new EndVsWinExperiment();
                            String [] args = {"10", "10", "5", "Configs2/config" + j + ".txt", "new-maze-25-50.txt"};
                            file.createNewFile();
                            FileWriter printer = new FileWriter(file);
                            Player wonPlayer = e.run(args);
                            printer.write(j + ", ");
                            if (wonPlayer instanceof FinishFirstPlayer) {
                                printer.write("1, ");
                                if (wonPlayer.getHighestIndex() == e.getBoardEnd()) {
                                    printer.write("1, ");
                                }
                                else {
                                    printer.write("0, ");
                            ...
                            if (wonPlayer instanceof JustPointsPlayer) {
                                printer.write("1, ");
                                if (wonPlayer.getHighestIndex() == e.getBoardEnd()) {
                                    printer.write("1, ");
                                }
                            ...
                            if (wonPlayer instanceof JustInFrontPlayer) {
                                printer.write("1, ");
                                if (wonPlayer.getHighestIndex() == e.getBoardEnd()) {
                                    printer.write("1, ");
                                }
    ...
    public EndVsWinExperiment() {
    }
    public int getBoardEnd() {
        return numberOfSpaces;
    }
    public Player run(String[] args) {
